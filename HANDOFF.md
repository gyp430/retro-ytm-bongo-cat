...
- **Packaged → portable Windows .exe** (§9) —
  `release/RetroYTM-BongoCat-<version>-portable.exe` (currently **1.0.6**),
...
| Radio autoplay quality (`≈`) | **done 2026-08-27; reworked 2026-08-28** — "radio v3": `/related` uses `get_watch_playlist(radio=True)`. **2026-08-28 fix — "queue fills with one artist":** `extendRadioNow` now (a) **only drains `state.originTracks` when it holds ≥4 distinct artists** (an artist/album page is one artist → skip straight to the station), (b) seeds the station from **up to 2 recent queue tracks, preferring ones whose artist ≠ the current track**, via `Promise.all` of two `/related` calls merged + shuffled, and (c) caps every refill at **2 per artist** (`diversifyTracks`). So the station drifts outward instead of orbiting the seed artist. **2026-08-31 fix — "same song floods the queue" (niche genres, e.g. Initial D / eurobeat):** added title-identity dedup — `normTitle(title, artists)` de-accents, drops the artist name if it's baked into the title, strips bracketed / "feat" / noise tokens (`extended`, `remix`, `nightcore`, `sped up`, `radio edit`, `eurobeat`, `initial d`, `official video`, …) and all non-alphanumerics, so "Deja Vu", "Deja Vu (Extended Mix)", "Deja Vu (Eurobeat Remix)" collapse to one key. Session `radioTitleSeen` set + `dedupByTitle(list, dropSessionSeen)` filter both radio branches; `diversifyTracks` now buckets artist-less tracks by title so they can't skip the per-artist cap; a niche seed that title-filters down to <3 candidates relaxes to video-id-only so radio never stalls. See §5c + §3. |
...
- **LCD time readout — elapsed / remaining / both** (2026-08-30). Was a boolean
  `state.showRemaining`; now `state.timeMode` ∈ `elapsed` / `remaining` / `both`,
  persisted as `retro.timeMode`. Clicking `#lcd-time` cycles the three;
  `⚙` → Playback → *Time readout* (`#set-time-mode`) is the same setting (kept in
  sync). `paintTime()` renders `M:SS` / `-M:SS` / `M:SS / -M:SS`; **both** adds a
  `.lcd--both` class (`winamp.css`) that drops the font to 13px so both times fit
...
`version` drives the filename; currently **1.0.6**. Full QA instructions:
**`QA.md`**.
